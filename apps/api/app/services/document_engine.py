...
def _classify_clause(text: str) -> ClauseType:
    ...
    if any(
        re.search(rf"\b{re.escape(w)}\b", first_line)
        for w in ["term", "terms", "renewal", "expiration", "duration"]
    ):
    ...
    if any(
        w in lower
        for w in ["governing law", "jurisdiction", "governed by", "construed in accordance"]
    ):
        return ClauseType.DISPUTE_GOVERNING_LAW
    if any(
        w in lower
        for w in ["maximum liability", "limitation of liability", "indemnif", "hold harmless"]
    ):
        return ClauseType.LIABILITY_INDEMNITY
    ...
    if any(
        ...
        for w in ["expire three", "expire", "renewal", "initial term", "extended term"]
    ):
        return ClauseType.TERM_RENEWAL
    if any(
        w in lower for w in ["confidential information", "non-disclosure", "nda", "proprietary"]
    ):
        return ClauseType.CONFIDENTIALITY_IP
