...
DATE_PATTERN = re.compile(
    r"\b((?:January|February|March|April|May|June|July|August|September|October|November|December)\s+\d{1,2},?\s+\d{4}|\d{1,2}[/-]\d{1,2}[/-]\d{2,4})\b",
    re.IGNORECASE,
)
AMOUNT_PATTERN = re.compile(
    r"\$\s*\d{1,3}(?:,\d{3})*(?:\.\d{2})?|\b\d{1,3}(?:,\d{3})*(?:\.\d{2})?\s*(?:Dollars|USD)\b",
    re.IGNORECASE,
)
    ...
    def extract_entities(self, document_id: str, clauses: Sequence[Clause]) -> list[Entity]:
        ...
        for clause in clauses:
            ...
            party_pattern = re.compile(
                r"\b([A-Z][a-zA-Z0-9\s]+(?:Inc\.|LLC|Ltd\.|Corporation|Company))\b"
            )
